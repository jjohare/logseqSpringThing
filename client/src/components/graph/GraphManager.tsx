...
import { graphDataManager, GraphData, Node as GraphNode, Edge as GraphEdge } from '../../lib/managers/graph-data-manager'
...
const GraphManager = () => {
  ...
  const nodes = useRef<THREE.InstancedMesh | null>(null)
  const [graphData, setGraphData] = useState<GraphData>({ nodes: [], edges: [] })
  const edges = useRef<Map<string, THREE.Line>>(new Map())
  ...
  const updateNodeInstances = (graphNodes: GraphNode[]) => {
  ...
  const updateEdges = (graphEdges: GraphEdge[], graphNodes: GraphNode[]) => {
  ...
  const updateNodePositions = (positions: Float32Array) => {
    if (!nodes.current || positions.length === 0) return
    ...
    for (let i = 0; i < positions.length; i += 4) {
      ...
      if (nodeIndex < 0 || nodeIndex >= nodes.current.count) continue
  ...
  const updateEdgePositions = () => {
    graphData.edges.forEach(edge => {
      if (!edges.current.has(edge.id)) return
  ...
  useEffect(() => {
    if (!nodes.current || !nodeSettings || !edgeSettings) return
    
    // Update node material
    if (nodes.current.material instanceof THREE.Material) {
      const material = nodes.current.material as THREE.MeshStandardMaterial
