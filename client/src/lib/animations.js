 ...
 */
export function createTransition(properties = ['all'], duration = timingPresets.normal, timingFunction = timingFunctions.easeInOut, delay = 0) {
    return properties
 ...
 */
export function useAnimatedVisibility(visible, duration = timingPresets.normal, delay = 0) {
    const [shouldRender, setShouldRender] = useState(visible);
    const [animationState, setAnimationState] = useState(visible ? 'entered' : 'exited');
    useEffect(() => {
        let timeoutId;
        if (visible) {
 ...
 */
export function usePerformanceAwareAnimation(preferredAnimation = 'fade') {
    ...
    useEffect(() => {
        ...
        if (navigator.hardwareConcurrency <= 2 || // CPU cores      
            ('deviceMemory' in navigator && navigator.deviceMemory <= 2) // RAM in GB, not available in all browsers
        ) {
    ...
    if (reducedMotion)
        return 'none';
    if (lowPerformance)
        return 'fade'; // Simplest animation for low-end devices
